...
> 이 프로젝트의 포지션: SECS/GEM 경험 없이 3일 만에 구현 → 빠른 학습 능력 어필  
> 핵심 메시지: "몰랐지만 표준 문서를 읽고 직접 만들었다"

---

## 면접 서사 구조 (오프닝)

> "저는 SECS/GEM 실무 경험이 없습니다.  
> 하지만 이 포지션에 지원하면서 SEMI E5/E30/E37 표준 문서를 직접 읽고,  
> 3일 만에 Host-Equipment 간 전체 통신 흐름이 동작하는 데모를 만들었습니다.  
> 새로운 기술 도메인에서 어떻게 학습하고 구현하는지 보여드리고 싶었습니다."
...
이 프로젝트의 버그 중 하나: RPTID=1003의 CHAMBER_TEMP는 엄밀히 SVID인데
코드에서 DVID로 처리하고 있습니다. 실제 업무에서는 Report 정의 시 구분이 필요합니다.

---

### Q6. "이 프로젝트의 한계가 뭔가요?" (자기 인식 질문)

이 질문이 나오면 솔직하게 말하는 것이 오히려 강점입니다.
...
3. **Trace 차트 미완**: 실시간 공정 데이터(온도/유량/압력)가 로그에만 찍히고 차트가 없습니다. Day 3 작업으로 남겨둔 상태입니다.
4. **영속화 없음**: In-Memory로만 동작해서 프로세스 재시작 시 데이터가 사라집니다.
...
> "SECS/GEM은 이번에 처음 접했지만, 표준 문서를 읽고 구현하는 과정에서
> MES가 장비와 통신하는 방식의 원리를 직접 경험했습니다.
> Critical Manufacturing 플랫폼도 같은 방식으로 접근할 수 있다고 생각합니다.
> 9년간 MES 업무를 하면서 쌓은 공정 흐름과 데이터 수집에 대한 이해를
> 새로운 플랫폼 위에서 빠르게 구현하는 것이 제 강점입니다."
